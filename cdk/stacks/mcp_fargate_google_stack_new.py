...
class McpFargateGoogleStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        # --- Container image -------------------------------------------
        # Pointed at the prod-style server (data_reports_mcp_server) — the
        # merged Dockerfile inlines the azureagent_infra base layers so the
        # POC builds the same way prod will. Swap back to
        # "../lambdas/mcp_fastmcp_server" to validate the original POC server.
        ...
        # 3-month retention matches prod's investigation window for OAuth
        # bugs that surface days after the originating request.
        ...
        # Sized to match prod (and to give the heavy azureagent_infra-based
        # image room to start without OOMing).
        ...
        container = task_def.add_container(
            ...
            # No Docker HEALTHCHECK — prod relies solely on the ALB target
            # group health check (configured below). Keeping a single source
            # of truth for "is this task healthy" avoids divergent signals.
        )
        ...
        # Match prod: only DescribeTimeToLive granted here. DescribeTable is
        # NOT granted (prod doesn't grant it either — replicating that gap
        # in the POC so we can reproduce any first-use AccessDenied behavior
        # prod exhibits).
        ...
        # Match prod: CloudWatchLogsFullAccess is attached to the TASK role
        # (note: this is over-privileged — log writes already work via the
        # execution role — but mirror prod for parity testing).
        ...
        # Outbound locked to :443/tcp only — matches prod. If the server ever
        # needs another egress port (DB, custom API, NTP), explicitly add it
        # here rather than re-opening all-outbound.
        service_sg = ec2.SecurityGroup(
            self,
            "ServiceSg",
            vpc=vpc,
            description="Fargate service ingress (ALB only); egress :443 only",
        ...
        service_sg.add_egress_rule(
            ec2.Peer.any_ipv4(),
            ec2.Port.tcp(443),
            "HTTPS outbound (Google OAuth, AWS APIs via VPC interface endpoints, etc.)",
        )

        # --- Fargate service -------------------------------------------
        # min/max healthy percent left at CDK defaults (50/200) to match
        # prod's deploy behavior. At desired_count=1 this means the service
        # can briefly drop to 0 healthy tasks during rolling deploys.
        ...
        https_listener.add_targets(
            ...
            targets=[service],
            # Thresholds match prod: 60s interval / 30s timeout / 3 unhealthy
            # gives the heavy image enough margin during cold starts.
